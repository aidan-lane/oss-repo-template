# Lab 06 Report - Introduction to Open Source Software

## Testing NetworkX

**1.** Installed matplotlib and networkx

**2.** Running example output:\
<img src="ex_out.png" alt="drawing" width="500"/>
***
## Lab
...
def words_graph():
    """Return the words example graph from the Stanford GraphBase"""
    fh = gzip.open('words_dat4.txt.gz', 'r')
    words = set()
    for line in fh.readlines():
        line = line.decode()
        if line.startswith('*'):
...
def generate_graph(words):
    ...
    def edit_distance_one(word):
        for i in range(len(word)):
            ...
            j = lookup[c]
            for cc in lowercase[j + 1:]:
                new_word = left + cc + right
                
                for p in itertools.permutations(new_word, len(new_word)):
                    yield "".join(p)
    candgen = ((word, cand) for word in sorted(words)
               for cand in edit_distance_one(word) if cand in words)
    G.add_nodes_from(words)
